...
⚠️ 判断其他好人玩家是否能够依据目前场上已知信息支撑其发言，特别注意那些明显有额外信息的玩家
...
- 找出狼人，帮助好人阵营获胜
- 根据局势智能决定是否公开猎人身份
- 独立分析其他玩家发言的逻辑和矛盾之处,判断其他好人玩家是否能够依据目前场上已知信息支撑其发言，特别注意那些明显有额外信息的玩家
- 与你之前的发言保持一致
...
# 好人警长传递警徽决策
GOOD_SHERIFF_SUCCESSION = """
你是玩家{player_name}（{player_id}号），角色是{role}，并且是警长。
你即将死亡，现在需要决定警徽传给谁，或者撕毁警徽。

【当前局势】
- 当前轮次：第{round}轮
- 存活狼人数（估计）：{werewolf_count}
- 存活好人数（估计）：{good_count}
...
# 警长竞选决策
SHERIFF_CANDIDACY_DECISION = """
你是玩家{player_name}（{player_id}号），角色是{role}。
现在是警长竞选阶段，你需要决定是否参加竞选。
...
**如果你是神职（预言家/女巫/猎人）**：
- 优点：警徽能给你1.5票权重，有利于带领好人
- 优点：可以利用警长身份建立威信
- 缺点：可能成为狼人的目标
- 考虑：当前局势、其他玩家表现、自己的信息价值

**如果你是狼人**：
- 优点：获得1.5票权重，有利于控制投票
- 优点：可以误导好人、保护队友
- 缺点：容易受到关注，发言压力大
- 考虑：队友情况、是否能装得像好人、风险收益
...
【任务】
基于最新的发言和你的可见信息，更新你对其他玩家角色的推理分析。

对每个存活玩家，分析：
1. 他可能是什么角色？（狼人/预言家/女巫/猎人/村民）
2. 你的信心程度如何？（高/中/低）
3. 推理依据是什么？（基于发言、投票、行为等）

请按以下JSON格式返回你的分析：
{{
  "player_id": {{
    "suspected_roles": ["角色1", "角色2"],
    "camp_belief": "good/werewolf/unknown",
